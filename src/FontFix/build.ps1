...
$RequiredFiles = @(
        ...
        (Join-Path $GameDir "Brown Dust II_Data\Managed\UnityEngine.TextRenderingModule.dll"),
        (Join-Path $GameDir "Brown Dust II_Data\Managed\UnityEngine.InputLegacyModule.dll"),
        (Join-Path $GameDir "Brown Dust II_Data\Managed\UnityEngine.UI.dll"),
        (Join-Path $GameDir "Brown Dust II_Data\Managed\Unity.TextMeshPro.dll")
)
...
try {
    ...
    $Dll = Join-Path `
        $PSScriptRoot `
        "bin\Release\netstandard2.1\BD2VietnameseFontFix.dll"
    ...
    $PluginDir = Join-Path `
        $GameDir `
        "BepInEx\plugins\BD2VietnameseFontFix"

    New-Item -ItemType Directory -Force -Path $PluginDir | Out-Null

    Copy-Item `
        $Dll `
        (Join-Path $PluginDir "BD2VietnameseFontFix.dll") `
        -Force

    Write-Host ""
    Write-Host "Installed plugin:"
    Write-Host (Join-Path $PluginDir "BD2VietnameseFontFix.dll")
