...
try {
    ...
    $Dll = Join-Path `
        $PSScriptRoot `
        "bin\Release\netstandard2.1\BD2SkillTextExporter.dll"
    ...
    $PluginDir = Join-Path `
        $GameDir `
        "BepInEx\plugins\BD2SkillTextExporter"

    New-Item -ItemType Directory -Force -Path $PluginDir | Out-Null

    Copy-Item `
        $Dll `
        (Join-Path $PluginDir "BD2SkillTextExporter.dll") `
        -Force

    Write-Host ""
    Write-Host "Installed plugin:"
    Write-Host (Join-Path $PluginDir "BD2SkillTextExporter.dll")
